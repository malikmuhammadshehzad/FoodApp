...
export const CartSlice = createSlice({
    ...
    reducers:{
        ...
        addToCart:(state,action )=>{  // when user press add button on the first time, then we need to push these details(image , price etc) 
             ...
             if(itemPresent){ // if the item is present then we increase the quantity of that particular item 
                itemPresent.quantity++;
             }else{ // if the item is not present mean user click first time on the add button,
                state.cart.push({...action.payload,quantity:1}) //in that case just push all the detail of this single cart item   
             }
        },
        removeFromCart:(state,action)=>{ // here we are follow the method of filter, filtering the item  from cart, base on the ide of item inside of the cart
            const removeItem =state.cart.filter((item)=>item.id!== action.payload.id)
            state.cart=removeItem;
        },
        // we will apply this incrementQuantity on Plus button 
        incrementQuantity:(state, action)=>{
            const itemPresent =state.cart.find((item)=>item.id === action.payload.id) // here we  are going to check item is present in the card
            itemPresent.quantity++;
        },
